...
//    MIPLevel() = delete;
    ...
    const cv::Mat &getFirstMat() const {
        if (!levels_.empty()) return levels_[0];
    }
...
#endif
    // Методы и поля, специфичные для MIP-хвостов
};
...
#ifdef NDEBUG
    // Вывод информации об исходном изображении
    std::cout << "Ширина: " << mipLevel_.getWidth() << std::endl;
    std::cout << "Высота: " << mipLevel_.getHeight() << std::endl;
    std::cout << "Количество каналов: " << mipLevel_.getChannels() << std::endl;
    std::cout << "Глубина: " << mipLevel_.getDepth() << std::endl;
#endif
...
#ifdef NDEBUG
    std::cout << "Start MipLevels " << std::endl;
#endif
    for (int i = 0; i < mipLevel_.level_size(); i++) {
...
#ifdef NDEBUG
    std::cout << "Debug pagesData " << std::endl;
    pagesData_.print();
#endif
...
#if 0
    std::ifstream file(path_image, std::ios::binary);
    if (file) {
...
//    Вычисление количества MIP-уровней и заполнение соответствующего поля заголовка:
    header.mipLevelCount = std::log2(std::max(mipLevel_.getFirstMat().cols, mipLevel_.getFirstMat().rows)) + 1;
//    Вычисление индекса первой страницы для каждого MIP-уровня и заполнение соответствующего поля заголовка:
    ...
    for (int i = 0; i < mipLevel_.level_size(); i++) {
        ...
        auto pageCount = std::ceil(static_cast<double>(mipWidth) / pagesData_.getPageWidth()) *
    ...
    mipTail.set_tailOfSet( sizeof(Header) + (header.mipLevelPageIndex[mipTail.get_tail_start()] * pagesData_.getPageSize()));
    header.mipTailOffset = mipTail.get_tailsOfSet();

    for (int i = mipTail.get_tail_start(); i < mipLevel_.level_size(); i++) {
        auto mipWidth = mipLevel_.getFromIndex(i).cols;
        auto mipHeight = mipLevel_.getFromIndex(i).rows;
        auto pageCount = std::ceil(static_cast<double>(mipWidth) / pagesData_.getPageWidth()) *
                         std::ceil(static_cast<double>(mipHeight) / pagesData_.getPageHeight());
